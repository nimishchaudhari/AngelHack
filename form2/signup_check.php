...
$emailcheck_u="SELECT * FROM doctor where email='$email'";
...
{
$sql="INSERT INTO doctor(name,email,password,signup_date,ip,location,zip,phone,hospital,type) values('$username $lastname','$email','$password',now(),'$ip','$country $state $city','$zip','$phone','$hospital','$type')";
...
$check_1="SELECT * FROM doctor WHERE email='$email' and password='$password'";
$pass_check_1=mysqli_query($connection,$check_1);
$fetch_id_sql=mysqli_fetch_array($pass_check_1);
$fetch_id=$fetch_id_sql['dr_id'];
